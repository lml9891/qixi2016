...
$(function () {
                        ...
                        $loadingBg.addClass('hideAnim').on('webkitAnimationEnd', function () {
                            ...
                            get_user_new_content(function (data) {
                                if (data.Success) {
                                    if (data.Data.ID == -1) {
                                        //默认20条
                                        $('#indexFontGoContent').html(data.Data.Content);
                                    } else {
                                        //
                                        $('#indexFontGoContent').html('To ' + data.Data.LoveTo + '<br />' + data.Message + '<br />' + 'From ' + data.Data.LoveFrom);
                                    }
                                }
                            });
    ...
    function init() {
        ...
        //点击大楼按钮
        $('.E_Big').on(od, function () {
            toBig(true);
            setTimeout(function () {
                window.location.href = 'good.aspx';
            }, 4000);
        });
